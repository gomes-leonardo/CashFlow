...
public class GenerateExpenseReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        {
            var expenseTable = CreateExpenseTable(page);

            var row = expenseTable.AddRow();
            row.Height = HEIGHT_ROW_EXPENSE_TABLE;

            AddExpenseTitle(row.Cells[0], expense.Title);
            AddHeaderForAmount(row.Cells[3]);

            row = expenseTable.AddRow();
            row.Height = HEIGHT_ROW_EXPENSE_TABLE;
            ...
            {
                var descriptionRow = expenseTable.AddRow();
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Author = "Leonardo Rodrigues";
